...
if(isset($_POST["token"]) && password_verify("dataget", $_POST["token"]))
{
	...
	if($backend=="all")
	{
		...
		$srno=0;
		$query=$db->prepare('SELECT *,count(*) as count FROM errors500 WHERE DATE(timestamp) >= NOW() - INTERVAL ? DAY group by source Order BY count DESC');
		$data=array($time);
		$execute=$query->execute($data);
		if($execute)
		{
	...
	{
		$srno=0;
		$query=$db->prepare('SELECT *,count(*) as count FROM errors500 WHERE backend=? AND DATE(timestamp) >= NOW() - INTERVAL ? DAY group by url Order BY count DESC');
		...
		$srno=0;
		$query=$db->prepare('SELECT *,count(*) as count FROM errors500 WHERE backend=? AND DATE(timestamp) >= NOW() - INTERVAL ? DAY group by source Order BY count DESC');
		$data=array($backend,$time);
		$execute=$query->execute($data);
		if($execute)
		{
